...
describe('auth routes', () => {
  ...
  it('start returns Google authorization URL, stores state, and accepts allowlisted redirect', async () => {
    const app = createApp({ sessionSecret: SECRET, googleClientId: CLIENT_ID, appOrigins: [APP_ORIGIN] });
    const res = await app.request(`/api/auth/oauth/google/start?redirect=${encodeURIComponent(`${APP_ORIGIN}/account`)}`);
  ...
  it('rejects callback without a matching OAuth state', async () => {
    const app = createApp({ sessionSecret: SECRET, googleClientId: CLIENT_ID, googleClientSecret: CLIENT_SECRET });
  ...
  it('callback verifies Google token, sets session cookie, and /me returns profile', async () => {
    ...
    const start = await app.request(`/api/auth/oauth/google/start?redirect=${encodeURIComponent(`${APP_ORIGIN}/account`)}`);
    ...
    const cb = await app.request(`/api/auth/oauth/google/callback?code=abc&state=${encodeURIComponent(state)}`, {
    ...
    expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('oauth2.googleapis.com/tokeninfo'), expect.anything());

    const sessionMatch = cookie.match(/gys_session=([^;]+)/);
    expect(sessionMatch?.[1]).toBeTruthy();
    const me = await app.request('/api/auth/me', { headers: { cookie: `gys_session=${sessionMatch?.[1]}` } });
