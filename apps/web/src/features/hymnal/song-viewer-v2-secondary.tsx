  ...
  return (
    <div className="song-v2-secondary" aria-label={labels.controls}>
      <div className="song-v2-controlgroup" role="group" aria-label="Notasi chord">
        <button
          type="button"
          className={accidentalMode === 'sharp' ? 'active' : ''}
          aria-pressed={accidentalMode === 'sharp'}
          onClick={() => onAccidental('sharp')}
        >
          ♯ {labels.sharp}
        </button>
        <button
          type="button"
          className={accidentalMode === 'flat' ? 'active' : ''}
          aria-pressed={accidentalMode === 'flat'}
          onClick={() => onAccidental('flat')}
        >
          ♭ {labels.flat}
        </button>
      </div>
      <ViewerStepper
        label={`${labels.key} ${transposeStep > 0 ? `+${transposeStep}` : transposeStep}`}
        minusLabel={labels.lower}
        plusLabel={labels.raise}
      ...
      {mode === 'pdf' && (
        <>
          <div className="song-v2-controlgroup" role="group" aria-label="Jumlah halaman">
            <button
              type="button"
              className={pageMode === 1 ? 'active' : ''}
              aria-pressed={pageMode === 1}
              onClick={() => onPageMode(1)}
            >
              {labels.onePage}
            </button>
            <button
              type="button"
              className={pageMode === 2 ? 'active' : ''}
              aria-pressed={pageMode === 2}
              disabled={pageCount <= 1}
              onClick={() => onPageMode(2)}
            >
              {labels.twoPages}
            </button>
          </div>
          ...
          <ViewerStepper
            ...
            minusLabel={labels.decreaseScore}
            plusLabel={labels.increaseScore}
            onMinus={() => onZoom(-0.1)}
            onPlus={() => onZoom(0.1)}
          />
        </>
