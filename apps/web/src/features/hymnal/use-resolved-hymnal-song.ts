...
export function useResolvedHymnalSong({ book, song, accidentalMode, transposeStep }: { book: string; song: string; accidentalMode: AccidentalMode; transposeStep: number }) {
  ...
  useEffect(() => {
    ...
    setHymnalPlayerTrack({ key: `${book}:${song}`, url: resolved.midiUrl, title }, { accidentalMode, transposeStep });
