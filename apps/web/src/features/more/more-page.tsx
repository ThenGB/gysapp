...
const EXTERNAL = [
  ...
  { href: 'https://tjc.org/id/pujian/pujian-padus', label: 'Pujian / Paduan Suara', icon: Broadcast },
  ...
  { href: 'https://www.youtube.com/channel/UCnKhYlQA5iJJvobPF4IYJFQ', label: 'Podcast', icon: YoutubeLogo },
  ...
  { href: 'https://www.youtube.com/channel/UCAHSLvPBcg2M-_N1VQfhxrg', label: 'YouTube', icon: YoutubeLogo },
  { href: 'https://open.spotify.com/show/4edDo52t3IlkgiWhBnk1GK', label: 'Spotify', icon: SpotifyLogo },
];
    ...
    <div className="content-shell more-page">
      ...
      <Section title="Literatur & Bacaan">
        <ul className="more-grid">
          {LITERATURE.map((item) => (
            <li key={item.kind}><Link to={`/literature/${item.kind}`} className="more-card"><item.icon size={26} aria-hidden="true" /><span>{item.label}</span></Link></li>
          ))}
        </ul>
      </Section>
      ...
      <Section title="e-GYS & Media">
        <ul className="more-grid">
          {EXTERNAL.map((item) => (
            <li key={item.href}><button type="button" className="more-card" onClick={() => openExternal(item.href)}><item.icon size={26} aria-hidden="true" /><span>{item.label}</span></button></li>
          ))}
        </ul>
      </Section>
      ...
      <Section title="Catatan & Koleksi">
        <ul className="more-grid">
          {[
            ...
            <li key={item.to}><Link to={item.to} className="more-card"><item.icon size={26} aria-hidden="true" /><span>{item.label}</span></Link></li>
          ))}
        </ul>
      </Section>
      ...
      <Section title="Unduhan & Pengaturan">
        <ul className="more-grid">
          <li><button type="button" className="more-card" disabled><DownloadSimple size={26} aria-hidden="true" /><span>Versi Alkitab & Buku Kidung</span></button></li>
          <li><Link to="/settings" className="more-card"><Gear size={26} aria-hidden="true" /><span>Pengaturan</span></Link></li>
          <li><Link to="/account" className="more-card"><UserCircle size={26} aria-hidden="true" /><span>Akun</span></Link></li>
          <li><Link to="/report" className="more-card"><PaperPlaneTilt size={26} aria-hidden="true" /><span>Kirim Masukan</span></Link></li>
        </ul>
      </Section>
    </div>
  );
